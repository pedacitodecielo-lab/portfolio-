...
import { ExternalLink, Github, Database, BarChart2, Zap, Search, Activity, GitBranch } from 'lucide-react'
            ...
            return (
              <FadeIn key={project.title} delay={i * 80}>
                <article className="card rounded-2xl overflow-hidden flex flex-col h-full group">
                  {/* Colored header */}
                  <div className={`bg-gradient-to-br ${project.headerBg} px-6 pt-6 pb-4 flex items-start justify-between`}>
                  ...
                  <div className="px-6 py-4 flex flex-col flex-1">
                    ...
                    <div className="flex gap-4 pt-4 border-t border-slate-100">
                      <a href={project.github} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-900 transition-colors font-semibold group/link">
                        <Github className="w-4 h-4 transition-transform group-hover/link:-translate-y-0.5" />
                        Code
                      </a>
                      <a href={project.live} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-blue-600 transition-colors font-semibold ml-auto group/link">
